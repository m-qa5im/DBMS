CREATE DATABASE CAMS_PROJECT
USE CAMS_PROJECT

CREATE PROCEDURE CreateCOVID19Tables
AS
BEGIN
...
EXEC CreateCOVID19Tables

-- Procdeure to display the data of specific table
CREATE OR ALTER PROCEDURE DisplayTableData
...
EXEC DisplayTableData @TableName = 'Continent'
...
EXEC DisplayTableData @TableName = 'Country'
...
EXEC DisplayTableData @TableName = 'Region'
...
EXEC DisplayTableData @TableName = 'DailyCovid19Reports'
...
EXEC DisplayTableData @TableName = 'VaccinationData'
...
EXEC DisplayTableData @TableName = 'TestingData'
...
EXEC DisplayTableData @TableName = 'PublicHealthMeasures'



-- Create procedure to drop tables if they exist
...
EXEC DropTablesIfExists


-- Dropping Procedures
DROP PROCEDURE IF EXISTS InsertVaccinationData;
DROP PROCEDURE IF EXISTS InsertTestingData;
DROP PROCEDURE IF EXISTS InsertPublicHealthMeasures;
DROP PROCEDURE IF EXISTS InsertContinent;
DROP PROCEDURE IF EXISTS InsertCountry;
DROP PROCEDURE IF EXISTS InsertDailyCovid19Reports;

-- This drops the entire database along with all its objects (tables, procedures, etc.)
DROP DATABASE IF EXISTS CAMS_PROJECT;

-- Adding a new column
ALTER TABLE Continent 
ADD TotalCountries INT;
...
-- Renaming Column
ALTER TABLE Continent 
ALTER COLUMN ContinentName NVARCHAR(255);

-- Renaming the table
EXEC sp_rename 'Region', 'StateProvince';
EXEC sp_rename 'StateProvince', 'Region';

-- Renaming Column
EXEC sp_rename 'Country.CountryName', 'CountryOfficialName', 'COLUMN';

-- Renaming Procedure
EXEC sp_rename 'InsertRegion', 'InsertStateProvince';
...
BEGIN
    DECLARE @SqlQuery NVARCHAR(MAX);
    SET @SqlQuery = 'DELETE FROM ' + QUOTENAME(@TableName) + ' WHERE ' + @Condition;
    EXEC sp_executesql @SqlQuery;
END;

EXEC DeleteData @TableName = 'Country', @Condition = 'CountryID = 1';
...
BEGIN
    DECLARE @SqlQuery NVARCHAR(MAX);
    SET @SqlQuery = 'UPDATE ' + QUOTENAME(@TableName) + ' SET ' + @SetClause + ' WHERE ' + @Condition;
    EXEC sp_executesql @SqlQuery;
END;

EXEC UpdateData @TableName = 'Country', @SetClause = 'TotalPopulation = 250000000', @Condition = 'CountryName = ''Pakistan''';


-- 	Query to check the total population by a continent
SELECT Continent.ContinentName, SUM(Country.TotalPopulation) AS TotalPopulation
FROM Continent 
JOIN Country  ON Continent.ContinentID = Country.ContinentID
GROUP BY Continent.ContinentName;

-- Total Cases, Deaths, and Recoveries by Date
SELECT Date, 
...
-- Average Testing Positivity Rate by Region
SELECT r.RegionName, 
       AVG((td.PositiveTests / td.TotalTests)) * 100 AS PositivityRate
FROM Region r
JOIN TestingData td ON r.RegionID = td.RegionID
GROUP BY r.RegionName;

-- Maximum Vaccinations Administered in a Single Day

SELECT MAX(TotalVaccinations) AS MaxVaccinations
FROM VaccinationData ;

--  Total Public Health Measures Implemented per Country

SELECT co.CountryName, 
       COUNT(phm.MeasuresID) AS TotalMeasuresImplemented
FROM Country co
JOIN Region r ON co.CountryID = r.CountryID
JOIN PublicHealthMeasures phm ON r.RegionID = phm.RegionID
GROUP BY co.CountryName;

-- 6.	Query to Check Top 5 Countries by Total Cases

SELECT TOP (5) c.CountryName,
       SUM(d.TotalCases) AS TotalCases
FROM Country c
JOIN Region r ON c.CountryID = r.CountryID
JOIN DailyCovid19Reports d ON r.RegionID = d.RegionID
GROUP BY c.CountryName
ORDER BY TotalCases DESC


-- Total Cases and Vaccinations by Country:
SELECT c.CountryName,
       SUM(d.TotalCases) AS TotalCases,
       SUM(v.TotalVaccinations) AS TotalVaccinations
FROM Country c
JOIN Region r ON c.CountryID = r.CountryID
JOIN DailyCovid19Reports d ON r.RegionID = d.RegionID
JOIN VaccinationData v ON r.RegionID = v.RegionID
GROUP BY c.CountryName
ORDER BY TotalCases DESC;
...
JOIN Region r ON c.CountryID = r.CountryID
JOIN DailyCovid19Reports d ON r.RegionID = d.RegionID
ORDER BY c.CountryName, d.Date;

-- identify Countries with Missing Vaccination Data
SELECT c.CountryName
FROM Country c
LEFT JOIN Region r ON c.CountryID = r.CountryID
LEFT JOIN VaccinationData v ON r.RegionID = v.RegionID
